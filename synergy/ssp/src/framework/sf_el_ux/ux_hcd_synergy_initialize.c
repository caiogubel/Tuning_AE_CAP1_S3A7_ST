...
static UINT  ux_hcd_synergy_initialize_common(UX_HCD *hcd)
{
    ...
    if (hcd -> ux_hcd_io == R_USBHS_BASE)
    {
        ...
        irq_ipl = SF_EL_UX_HCD_CFG_HS_IRQ_IPL;
    }
    else
    {
#endif
        if (hcd -> ux_hcd_io == R_USBFS_BASE)
        {
            ...
            irq_ipl = SF_EL_UX_HCD_CFG_FS_IRQ_IPL;
        }
        else
        {
            /* Error trap. */
    ...
    hcd_synergy->ux_hcd_synergy_transfer.ux_synergy_transfer_tx = NULL;
    hcd_synergy->ux_hcd_synergy_transfer.ux_synergy_transfer_rx = NULL;
...
UINT  ux_hcd_synergy_initialize_transfer_support(UX_HCD *hcd, const UX_HCD_SYNERGY_TRANSFER *p_transfer_instance)
{
    ...
    if (UX_SUCCESS == status)
    {
        ...
        if (hcd -> ux_hcd_io == R_USBFS_BASE)
        {
            if (hcd_synergy->ux_hcd_synergy_transfer.ux_synergy_transfer_tx->p_cfg->irq_ipl >= SF_EL_UX_HCD_CFG_FS_IRQ_IPL)
            {
                return (UINT)UX_CONTROLLER_INIT_FAILED;
            }
        }
        ...
        {
            if (hcd_synergy->ux_hcd_synergy_transfer.ux_synergy_transfer_tx->p_cfg->irq_ipl >= SF_EL_UX_HCD_CFG_HS_IRQ_IPL)
            {
                return (UINT)UX_CONTROLLER_INIT_FAILED;
            }
        }
...
static void ux_hcd_dma_complete_rx(transfer_callback_args_t * p_args)
{
    ...
    if ((ULONG)(UX_SYNERGY_HC_DFIFOSEL_CURPIPE_MASK & reg) >= 6UL)
    {
